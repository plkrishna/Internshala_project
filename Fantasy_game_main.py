...
"background-color: rgb(186, 189, 182);")
    ...
    def save_team(self):
        self.Db_cursor.execute('SELECT Name FROM teams WHERE Name="{}";'.format(self.New_team_name))
        checker=self.Db_cursor.fetchall()
        if checker!=[]:
            self.Db_cursor.execute('DELETE FROM teams WHERE Name="{}";'.format(self.New_team_name))
...
class Ui_Dialog(object):
    ...
    def b_accepted(self):
        ...
        ui.Ar.setCheckable(True)
        ui.Wk.setCheckable(True)
        ui.Bat.setChecked(False)
        ui.Bow.setChecked(False)
        ui.Ar.setChecked(False)
        ui.Wk.setChecked(False)
        ...
        ui.Wicketkeeper_num.setText(str(0))
        if ui.New_team_name=='':
            ui.create_message_box('Not a Valid Team name')
        else:
            self.Dialog.accept()
